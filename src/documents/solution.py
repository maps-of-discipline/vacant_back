...
class SolutionRenderer:
    ...
    def solution(
        ...
        template = f"/app/src/documents/templates/vm-solution-template.docx"
    ...
    def _get_rups(self, rup_data: GetRupDataResponseSchema):
        ...
        for i, discipline in enumerate(rup_data.target):
            if discipline.title in rup_data.choosen and (
                rup_data.choosen[discipline.title].variants.values()
            ):
                continue
